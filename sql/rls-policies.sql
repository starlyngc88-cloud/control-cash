...
-- Requiere: auth.uid() y una columna user_id en cada tabla

-- Función helper: el usuario actual
...
CREATE POLICY "admins_manage_allowed_users" ON allowed_users
  FOR ALL USING (
    EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.current_user_id() AND role = 'admin')
  );
