...
export async function getMovementsByBudgetCategory(budgetCategoryId: string, childIds: string[] = [], startDate?: string, endDate?: string) {
  ...
  return exps.map((e) => ({ ...e, people: peopleMap.get(e.person_id) ?? null }))
}
...
export async function getFinancialInsights(): Promise<FinancialInsight[]> {
  ...
  if (currentMb) {
    const currentCats = await getMonthCategories(currentMb.id, "")
    ...
    for (const cat of currentCats) {
      ...
      if (spent > cat.budgeted) {
        const currentExcess = spent - cat.budgeted
        let previousExcess: number | null = null
        if (prevMb) {
          const prevCats = await getMonthCategories(prevMb.id, "")
          const prevCat = prevCats.find((c) => c.name === cat.name)
          if (prevCat && prevCat.budgeted > 0) {
            const prevSpentAmount = prevSpent[prevCat.id] ?? 0
            if (prevSpentAmount > prevCat.budgeted) {
              previousExcess = prevSpentAmount - prevCat.budgeted
            }
          }
        }
        overspent.push({
