...
export default function PresupuestosPage() {
  ...
  const handleDeleteTemplate = async (id: string) => {
    if (!confirm("¿Eliminar esta plantilla y todos sus rubros?")) return
  ...
  const handleDeleteMonth = async (id: string) => {
    if (!confirm("¿Eliminar este mes financiero?")) return
    ...
    <div className="space-y-5">
      ...
      <div className="border rounded-lg overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2 bg-muted/30 border-b">
          <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Plantillas</span>
        ...
        {templates.length === 0 ? (
          <p className="text-xs text-muted-foreground p-4">No hay plantillas. Crea una para empezar.</p>
        ) : (
                ...
                <div key={tmpl.id} className="p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => { setSelectedTemplateId(tmpl.id); setOpenMonth(true) }}>
                        <Calendar className="size-3 mr-1" /> Mes
                      </Button>
                  ...
                  <div className="flex items-center gap-1.5 pt-0.5">
                    ...
                    <Button size="sm" variant="outline" className="h-7 text-xs shrink-0" onClick={() => handleAddCategory(tmpl.id)}>
                      <Plus className="size-3 mr-0.5" /> Rubro
                    </Button>
                  </div>
                </div>
              )
      ...
      <div className="border rounded-lg overflow-hidden">
        <div className="px-3 py-2 bg-muted/30 border-b">
          <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Meses financieros</span>
        </div>
        {monthlyBudgets.length === 0 ? (
          <p className="text-xs text-muted-foreground p-4">No hay meses financieros. Genera uno desde una plantilla.</p>
        ) : (
          <div className="divide-y">
        ...
        <DialogContent>
          ...
          <form onSubmit={handleCreateTemplate} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="name" placeholder="Ej: Plantilla Base" value={templateName} onChange={(e) => setTemplateName(e.target.value)} required />
            </div>
      ...
      <Dialog open={openCatEdit} onOpenChange={(v) => { if (!v) setEditingCat(null); setOpenCatEdit(v) }}>
        <DialogContent>
          <DialogHeader><DialogTitle>Editar rubro</DialogTitle></DialogHeader>
          <form onSubmit={handleEditCategory} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="catBudgeted">Monto</Label>
      ...
      <Dialog open={openMonth} onOpenChange={setOpenMonth}>
        <DialogContent>
          <DialogHeader><DialogTitle>Generar mes financiero</DialogTitle></DialogHeader>
          <form onSubmit={handleCreateMonth} className="space-y-4">
            ...
            <Button type="submit" className="w-full">Crear mes</Button>
          </form>
        </DialogContent>
      </Dialog>
    </div>
